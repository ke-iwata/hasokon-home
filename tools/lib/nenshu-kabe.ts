...
/**
 * 短時間労働者の社会保険の適用拡大では、**昼間部の学生は適用除外**（健康保険法第3条第9項）。
 * 夜間部・定時制・通信制、休学中の方は適用の対象になる。
 *
 * 判定ロジックからは外していない。`position: 'student'` は「親の扶養内・19〜22歳の学生」
 * という意味しか持たず昼間部とは限らないため、除外してしまうと夜間・定時制の方を
 * 取りこぼす。どちらか分からない以上、ロジックで断定せず注記で補う。
 */
...
export const HOKENRYO_CHOSEI_NOTE =
  '勤務先が保険料調整制度（2026年10月1日開始）を利用している場合、加入後の厚生年金保険料・健康保険料は最長3年、本人負担が25〜48%に軽減されます（対象は標準報酬月額12.6万円以下の方）。制度を使えるのは50人以下で新たに短時間労働者を加入対象にした勤務先などに限られ、事業主の申出制です。複数の勤務先で社会保険に加入している方・産休育休で保険料が免除されている方は対象外です。';
