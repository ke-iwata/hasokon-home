 ...
 *
 * ■ 源泉徴収税額の扱い（この計算機の設計の肝）
 * 毎月の源泉徴収は月額表・賞与の算出率表で決まるため、賞与の配分や月ごとの変動で
 * 年税額とはずれる。そのずれを外から当てにいかず、**給与明細の実額を主たる入力**にした。
 * 実額を受け取れば、月次の税額表に改正がどこまで織り込まれているかに関係なく、
 * 還付額を正しく出せる。
 *
 * 明細が手元にない人向けの推計モード（estimateWithheld）は、
 * 「扶養控除等申告書で申告した控除だけが毎月反映されていた」という前提を置く。
 * つまり社会保険料・基礎控除・配偶者控除・扶養控除・特定親族特別控除までは
 * 毎月引かれており、生命保険料控除・地震保険料控除・小規模企業共済等掛金控除・
 * 住宅ローン控除は年末調整で初めて効く、という置き方である。
 * 実際の源泉徴収額とは数千円〜数万円ずれるので、結果には必ず「概算」と明示すること。
 *
 * ■ 対象外
 ...
 * 源泉徴収税額の累計の推計。
 *
 * 毎月の源泉徴収は「扶養控除等申告書で申告した内容」だけが反映される。
 * 生命保険料控除・地震保険料控除・小規模企業共済等掛金控除・住宅ローン控除は
 * 年末調整で初めて効くので、それらを抜いた年税額を源泉徴収の累計とみなす。
 *
...
export function estimateWithheld(input: NenmatsuInput, socialInsurance: number): number {
  return calcYearTax(input, RULES_R8, socialInsurance, { skipYearEndDeductions: true }).yearTax;
}
