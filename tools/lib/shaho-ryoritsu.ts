 ...
 *
 * 根拠は2か所。1頁の ※1 が「対象となる保険料は、賞与に関するものを除いた
 * 厚生年金保険料・健康保険料です」と**列挙で**定めており、5頁「5．軽減される保険料額の
 * 計算方法」（Q5/A5）の注記が「賞与に関する保険料や、介護保険料、子ども・子育て支援金は、
 * 制度の対象外です」と**名指しで**除いている（2026-09-19 にPDFで確認）。
 * なおこのPDFは字間にスペースが入る組版なので、抽出したテキストを素のまま検索すると
 * 「介護保険料」は0件に見える（「介 護 保 険 料」で入っている）。
 * 雇用保険料は社会保険料ではないので
