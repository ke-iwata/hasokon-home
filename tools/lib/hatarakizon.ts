 ...
 * - **保険料率（健康保険・介護・子育て支援金・厚生年金・雇用保険）の一次情報は
 *   lib/shaho-ryoritsu.ts にまとめてある。**年度で変わる料率はあちらだけを見る
 *
 * 【データ更新箇所】年金の給付乗率が変わったら PENSION_ACCRUAL_RATE を、
 * 住民税の均等割が変わったら RESIDENT_PER_CAPITA を更新する。
 * **保険料率はここに持たない。** lib/shaho-ryoritsu.ts から import して同名で re-export
 * しているだけなので、料率が変わったらあちらだけを更新すればこのツールも追随する。
 * 同じ数字を2か所に置くと、片方だけが取り残される
 * （実際、令和8年度の改定で EMPLOYMENT_RATE がここと lib/furusato-nozei.ts で食い違った）
 */
...
/**
 * 保険料率（本人負担・労使折半後）は lib/shaho-ryoritsu.ts が持つ。
 * 既存の import 元（lib/tedori-keisan.ts・tests/）を変えずに済むよう、同名で re-export する。
 */
export {
  HEALTH_RATE,
  KAIGO_RATE,
