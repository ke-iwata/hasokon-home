 ...
 *    URLを組み立てるが、呼び出し元が持っている `usePathname()` の値は basePath（/tools）が
 *    取り除かれたパスなので、実際のURLと食い違う（docs/features/ga4-page-path.md）
 * 2. **本番ホスト以外へ1件も送らないこと**。test.hasokon.com と localhost の分が
...
describe('trackPageView', () => {
  ...
  it('gtag がまだ無いときは何もしない（読み込み前でも落ちない）', () => {
    calls = [];
    globals.window = { location: { href: 'https://hasokon.com/tools/', hostname: 'hasokon.com' } };
...
describe('送信先ホストの判定', () => {
  ...
  it.each([
    ['https://test.hasokon.com/tools/', 'テスト環境'],
    ['http://localhost:3000/tools/', 'ローカル開発'],
    ['http://127.0.0.1:3000/tools/', 'ローカル開発（IP）'],
    ['https://tool.hasokon.com/nenshu-kabe/', '旧サブドメイン'],
    ['https://tool.test.hasokon.com/nenshu-kabe/', '旧サブドメインのテスト環境'],
  ])('%s（%s）では送らない', (href) => {
    stubBrowser(href);
  ...
  it.each([
    ...
    ['trackEvent', () => trackEvent('tool_use', { tool: 'roulette' })],
    ['trackToolUse', () => trackToolUse('roulette', 'spin')],
    ...
    stubBrowser('https://test.hasokon.com/tools/roulette/');
  ...
  it('gtag.js のタグを出すかどうか（isAnalyticsEnabled）はホストを見ない', () => {
    ...
    stubBrowser('https://test.hasokon.com/tools/');
