...
| 毎年8〜10月（最低賃金） | 各労働局の答申 →**決定・官報公示**を追って `lib/saitei-chingin.ts` の `PREFECTURES` を更新する。答申の要旨の公示から15日の異議申出（最低賃金法11条2項）を経て決定・公示（同12条）されると発効日が確定するので、**公示日を明記している県だけ `decidedOn`** を入れ、発効日は `effectiveOn` に入れる。公示が確認できない県は厚労省の別紙の「発効日（予定）」を `plannedEffectiveOn` に入れる（**予定日では「発効済み」にしない**）。`DATA_CHECKED_AT` も毎回進める。出典は `node scripts/check-sources.mjs` で生存確認する |
