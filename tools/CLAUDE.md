...
- ツール37本（ほかに公開前が2本：`iryohi-kojo`・`taishokukin-tedori`（どちらも `stage: 'wip'`））/
  用途別ルーレット10本 / 使い方の記事6本 / テスト1657件
...
- **退職金は「10年ルール」と「重複期間の短縮」が別物**。2026年1月1日以後に支払を受ける
  退職金から、iDeCo一時金を先に受け取った場合の重複排除が「前年以前4年内」→
  **「前年以前9年内」**に延びた（所得税法施行令 第70条）。ただし重なった期間の全部を
  引くわけではなく、**前の一時金の額が当時の退職所得控除額に満たなければ、重複期間は
  「みなし勤続年数」まで短縮される**（同条）。iDeCoの一時金は掛金期間の控除額より
  小さいことが多いので、短縮を省くと控除を引きすぎて税額を多く出す。
  重複年数は利用者に聞かず、掛金期間と勤続期間の重なりから `overlapMonths()` が出す。
  **端数は所得税が1円未満切捨て・住民税が100円未満切捨て（市町村民税6%と道府県民税4%を
  それぞれ）**で、混ぜると100円ずれる（`lib/taishokukin.ts`。
