...
### 3-2. 404ページのカスタムエラーレスポンス（重要）

`npm run build` は `out/404.html` を出力しますが、CloudFront に設定しないと
存在しないURLで **S3のXMLエラーがそのまま表示されます**（OACのため403で返る）。
ディストリビューションの「エラーページ」タブで次の2つを作成します。

| HTTPエラーコード | エラーページのパス | HTTPレスポンスコード |
|---|---|---|
| 403 Forbidden | `/404.html` | 404 |
| 404 Not Found | `/404.html` | 404 |

403 も 404 に変換するのがポイントです。S3 + OAC では存在しないキーへのアクセスが
403 で返るため、403 を放置すると404ページに到達しません。
レスポンスコードを 200 にすると存在しないURLがインデックスされてしまうので、
**必ず 404 を返す**ようにします。
