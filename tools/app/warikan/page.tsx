...
export default function Page() {
  return (
    <>
      ...
      <h1>割り勘計算機</h1>
      <p className="lead">
        飲み会の合計金額と人数を入れるだけで、一人あたりの支払額を計算します。
        端数の丸め単位と、幹事が多めに払うか少なめに払うかも選べます。
      </p>
      ...
      <h2>このツールの計算方法</h2>
      <p>
        一人あたりの金額（合計 ÷ 人数）を、選んだ丸め単位で切り捨てまたは切り上げたものを
        「参加者（幹事以外）の支払額」とし、残りを幹事が支払う仕組みです。
        どのモードでも「参加者の支払額 ×（人数 − 1）+ 幹事の支払額 = 合計金額」が
        必ず成り立つように計算しているので、集金額が合わなくなる心配がありません。
      </p>
      <ul>
        <li>
          <strong>幹事が端数を負担</strong>: 参加者は切り捨てでキリのいい金額を払い、
          不足分を幹事が多めに支払います。
        </li>
        <li>
          <strong>幹事が端数を得</strong>: 参加者は切り上げで少し多めに払い、幹事の支払いは少なくなります。
          集めすぎた場合は「お釣り」として表示します。
        </li>
        <li>
      ...
      <div className="note">
        表示される金額は計算上の目安です。実際の集金・精算の方法（端数やお釣りの扱いなど）は、
        参加者間で事前に合意のうえご利用ください。
      </div>
