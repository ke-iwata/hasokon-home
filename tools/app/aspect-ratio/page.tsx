...
export default function Page() {
  return (
    <>
      ...
      <h1>アスペクト比計算機</h1>
      <p className="lead">
        幅と高さ(px)を入れるだけで、アスペクト比を最簡分数（16:9など）で計算します。
        比率を保ったままのサイズ変換や、CSSのコピー用コードにも対応しています。
      </p>
      ...
      <h2>アスペクト比とは</h2>
      <p>
        アスペクト比は、画面や画像の「幅と高さの比率」のことです。例えば1920x1080の動画は、
        両方を120で割ると16:9になります。同じ16:9なら1280x720でも3840x2160（4K）でも見た目の縦横バランスは同じで、
        画質（解像度）だけが変わります。動画編集・サムネイル作成・Webデザインでは、
        この比率を揃えることで黒帯や意図しないトリミングを防げます。
      </p>
      ...
      <div className="note">
        SNSの推奨サイズは各プラットフォームの仕様変更で変わることがあります。
        入稿・投稿前に各サービスの最新の公式ヘルプをご確認ください。
      </div>
