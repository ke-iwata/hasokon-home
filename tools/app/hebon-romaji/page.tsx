...
export default function Page() {
  return (
    <>
      ...
      <p className="lead">
        姓・名をひらがなまたはカタカナで入力すると、パスポート申請で使うヘボン式ローマ字（大文字）に変換します。
        撥音「ん」・促音「っ」・長音の表記ルールに対応しています。
      </p>
      ...
      <p>
        パスポートの氏名は、戸籍のふりがなをヘボン式ローマ字で表記するのが原則です。
        ヘボン式は英語の発音に近づけた表記法で、「し=SHI」「ち=CHI」「つ=TSU」「ふ=FU」「じ=JI」のように書きます。
        学校で習う訓令式（し=SI、ち=TI）とは異なるため、注意が必要です。
      </p>
      ...
      <p>
        「おう」「おお」の長音を含む氏名は、申請時にO表記（ONO）かOH表記（OHNO）かを選べます。
        ただし、一度パスポートに記載した表記はその後の申請でも引き継がれ、原則として変更できません。
        クレジットカードや航空券のマイレージ登録など、他の名義表記との統一も考えて選びましょう。
      </p>

      <div className="note">
        本ツールは一般的なパスポート用ヘボン式ローマ字のルールにもとづく参考表示です。
        非ヘボン式表記（例: 外国式の綴り）を希望する場合や表記に迷う場合など、正式な確認は各都道府県のパスポート申請窓口へお問い合わせください。
      </div>
