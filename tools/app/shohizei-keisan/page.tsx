...
// 日本語の検索結果で表示されるのは全角30字前後（layout.tsx 参照）。
// 「（予定）」を後ろに置くとスニペットで切れ、まさに仕様書が避けたかった
// 「成立済みの事実として読まれる」形になるので、前半30字に入る位置に置く。
const title = PENDING
  ? '消費税 計算機・軽減税率チェッカー｜食料品1%（予定）の軽減額と税込税抜の計算'
  : '消費税 計算機・軽減税率チェッカー｜食料品1%の軽減額と税込税抜の計算';
...
const faq = [
  ...
  {
    q: 'なぜ0%ではなく1%なのですか？',
    // 「10〜12か月」「5〜6か月」の具体的な月数は官邸の会見には無く、報道が伝えた
    // 見込みなので書かない（このページの出典は一次情報だけ。レビューでの指摘）
    a: '政府は、事業者のシステム改修に必要な期間を理由として挙げています。1%にすれば0%にするよりも改修の期間を短くできる、という説明です。1%相当の負担については、中低所得の勤労者への給付（給付付き税額控除）で実質的に軽くする設計とされています。このツールは政策の是非を判断するものではありません。',
  },
  {
    q: '外食も1%になりますか？',
