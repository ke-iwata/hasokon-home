...
const faq = [
  ...
  },
  {
    ...
    a: '1円未満の端数をどう処理するかの違いです。切り捨ては下に落とす（0.9円→0円）、四捨五入は0.5円で上に上がる（0.5円→1円、0.4円→0円）、切り上げは上に押し上げる（0.1円→1円）。同じ「199円の3%オフ」でも、193円（切り捨て・四捨五入）と194円（切り上げ）で分かれます。端数がちょうど0.5になる「150円の3%オフ」なら、145円（切り捨て）・146円（四捨五入・切り上げ）と3方式が分かれます。店舗ごとに実運用が違うため、レジと1円合わないときはこのツールで丸め方を切り替えると、どの方式で処理されているかが分かります。既定は四捨五入です。',
  },
  {
...
export default function Page() {
  return (
    <>
      ...
      <p>
        1円未満の端数を店舗ごとにどう丸めるかは統一ルールがありません。<strong>切り捨て・四捨五入・切り上げ</strong>のいずれで処理するかで、同じ割引率でも1円ずれることがあります。
      </p>
      <ul>
        <li>
      ...
      <div style={{ overflowX: 'auto' }}>
        {/*
          セル・見出しは1行に固定する。狭幅で「10%」と「オフ」・「1,000」と「円」の
          2行折り返しが起きると表がガタつく（外側の overflow-x で横スクロールにする）。
        */}
