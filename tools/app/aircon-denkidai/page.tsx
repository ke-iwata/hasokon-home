...
export default function Page() {
  return (
    <>
      ...
      <h1>エアコン電気代 計算機</h1>
      <p className="lead">
        エアコンの消費電力と使用時間から、1時間・1日・1ヶ月あたりの電気代の目安を計算します。
        部屋の広さ（畳数）から消費電力を選ぶこともできます。
      </p>
      ...
      <p>
        単価のデフォルト31円/kWhは、全国家庭電気製品公正取引協議会が定める「電力料金目安単価」（2022年7月改定）です。
        実際の単価は契約している電力会社・プランや燃料費調整額によって異なるため、検針票などで確認して入力するとより正確になります。
      </p>

      <h2>実際の電気代は「定格×時間」よりも安くなることが多い</h2>
      <p>
        カタログに書かれている消費電力（定格消費電力）は、フルパワーで運転したときの値です。
        実際のエアコンはインバーター制御によって出力を細かく調整しており、部屋が設定温度に達したあとは定格の数分の一の電力で運転します。
        そのため、つけ始めは計算値に近く、安定運転中はずっと安くなるのが普通です。
        逆に、猛暑日や真冬など外気温との差が大きい日、断熱性の低い部屋では消費電力が増えます。
        本ツールの結果は「定格で動き続けた場合の上限に近い目安」として使ってください。
      </p>

      <div className="note">
        計算結果は目安です。実際の電気代は機種・設定温度・外気温・部屋の断熱性能・契約プラン（段階料金・燃料費調整額・再エネ賦課金）によって変わります。
        畳数プリセットの消費電力は主要メーカーの冷房時の定格消費電力の目安であり、暖房時はこれより大きくなるのが一般的です。
      </div>
