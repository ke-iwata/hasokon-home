...
const faq = [
  ...
  },
  {
    ...
    a: '原則として両親がともに14日以上の育児休業（出生時育児休業給付金または育児休業給付金が支給される休業）を取ることが要件ですが、本人と配偶者で見る期間が違います。本人は「対象期間」内に14日以上で、この対象期間は、産後休業をしない親（父など）は「子の出生日または出産予定日のうち遅い日から8週間を経過する日の翌日」まで、産後休業をする親（出産した本人）は同じ起算日から16週間を経過する日の翌日までです（始まりはどちらも出生日と出産予定日のうち早い日）。配偶者のほうは、本人が産後休業をする場合でも「出生日等から8週間を経過する日の翌日まで」の期間で14日以上を見ます。配偶者がいない、配偶者が無業者、自営業者やフリーランスなど雇用される労働者でない、配偶者が産後休業中といった場合は、本人の休業だけで要件を満たします。要件を満たすかどうかはハローワークが決定するので、必ずハローワークまたは勤務先で確認してください。',
  },
  {
    q: 'いつ振り込まれますか？',
  ...
  },
  {
    ...
    a: '一支給単位期間の就業が10日（10日を超える場合は80時間）以下であることが支給の要件です。事業主から賃金が支払われた場合、その額が「賃金日額×休業期間の日数」の13%（育児休業の開始から181日目以降は30%）以下なら減額されず、そこから80%未満までなら「80%との差額」が育児休業給付金の額になり、80%以上だと支給されません。出生後休業支援給付金のほうは、育児休業給付金が支給される限り減額されません。この計算機は給与が支払われない前提で計算しています。',
  },
];
...
export default function Page() {
  return (
    <>
      ...
      <h2>対象期間は、産後休業をするかどうか（父と母）で違います</h2>
      <p>
        13%が上乗せされる日は、休業ならいつでもよいわけではなく
        <strong>対象期間の中の休業</strong>に限られます。この対象期間の長さが、
        <strong>産後休業をするかどうかで変わります。</strong>
        期間の始まりはどちらも「子の出生日<strong>または</strong>出産予定日のうち
        <strong>早い日</strong>」で、終わりだけが次のように分かれます。
      </p>
      <ul>
        <li>
          <strong>産後休業をしない親（父など）</strong>
          … 子の出生日<strong>または</strong>出産予定日のうち遅い日から起算して
      ...
      <p>
        <strong>始まりと終わりで見る日が違う</strong>点に注意してください。予定日より早く
        生まれた場合、期間は出生日から始まって予定日基準で終わるので、そのぶん長くなります。
        計算機の「出産予定日」は、出生日と違うときだけ入れてください。
      </p>
      <p>
        なお<strong>配偶者の14日以上は、本人が産後休業をする場合でも
        「出生日等から{SHUSSHOGO_WEEKS_DEFAULT}週間を経過する日の翌日まで」の期間で見ます</strong>
        （本人の対象期間が{SHUSSHOGO_WEEKS_POSTPARTUM}週間でも、配偶者側の窓は
        {SHUSSHOGO_WEEKS_DEFAULT}週間のままです）。
      </p>
      <p>
