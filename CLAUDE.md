...
出したPRは**マージかクローズまで**自分で見届ける。手順は
[.claude/skills/pr-watch/SKILL.md](./.claude/skills/pr-watch/SKILL.md)。

- **ティックのたびにGitHubから取り直す。** 前回の読みを信じない
- **起票・push の直後に状態をまとめない。** 相手の指摘はその1〜16分後に届く
  （2026-09-24〜25 に6本が11〜15時間止まった原因がこれ。毎時の確認は生きていたのに、
  4〜7件の指摘を「0件」と報告し続けた）
- **「やることが見つからない」は止める理由にならない。** PRがopenで最後のイベントが
  自分のものでないなら、自分の番
