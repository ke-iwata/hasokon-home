...
前者は利用者が給与明細で持っている。実額を受け取れば、月次の源泉徴収税額表に
改正がどこまで織り込まれているか（仕様書の「要確認事項」・未確認のまま）に
関係なく正しい額が出せる。推計モードは明細が手元にない人向けの補助と位置づけ、
「扶養控除等申告書で申告した控除だけが毎月反映されている」という前提を画面に明記した。
