...
**状態**：一部実施（2026-09-16 起票）。**A（監査スクリプトの内訳出力・週次実行）だけ先に入れた。
B・C・D は未着手** — 運営者の手作業（Search Console の画面確認）の結果で内容が変わるため。
本ファイルの「まず運営者がやること」を済ませたら、結果をこのファイルの末尾「経過」に追記する。
...
- 2026-09-16：**A を実施。** `scripts/gsc-canonical-audit.mjs` に `coverageState` 別の
  ...
  **B・C・D はここでは触っていない。** B（16ページの `noindex`）は公開中のページを検索から
  引っ込める後戻りしにくい変更で、原因が「手動による対策」だった場合は打ち手が変わる。
  上の「まず運営者がやること」1〜2 の結果を待つ
