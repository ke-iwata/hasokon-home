...
- 2026-09-26：**アドレス変更（`tool.`・`game.`・`roulette.` → `hasokon.com`）は取り消さず、続ける**（運営者判断）。
  理由は「目先の利益より長期的な視点で」。3 本を 1 か所にまとめる形は公式ヘルプの推奨から外れるが、
  取り消しで戻るものは期待しにくく（旧 URL の評価はすでに消えている）、移転先が hasokon.com であることを
  Google に伝え続けるほうを採る。301 はそのまま。
  **09-25 の「取り消すかは運営者判断待ち」はこれで閉じる。** 申請は 2027-02 上旬に 180 日を迎え、
  **アドレス変更ツールによるシグナルの受け渡しはそこで終わる**
  （[アドレス変更ツール](https://support.google.com/webmasters/answer/9370220)）。
  ただし **301 自体はその後も評価を渡し続ける**ので、**301 は恒久的に残す（外さない）**。
  **この判断は見直さない**：180 日で自然に終わるので、取り消しは今後の選択肢に入れない。
  10-01 の再計測（上の「降り口」）で登録が動かなくても、移り先は品質判定側・Bing・`public` 停止の判断であって、
  アドレス変更の取り消しではない
- 2026-09-26：**v1.21.0 を本番リリース**（B の 16 本 noindex〔#257〕と本文の薄いツール 4 本〔#259〕を含む）。
  本番で `/tools/r/lunch/` の `noindex, follow` と、tools のサイトマップに `/r/`・`/guide/` が 0 件なことを確認。
  同日、Search Console の「クロール済み - インデックス未登録」（30 件）で**「新しい検証を開始」**
  （前回は 08-27 開始・09-05 不合格）。開始時点で保留 30・失敗 0。
  **結果は検証の画面と 10-01 の再計測で見る**
