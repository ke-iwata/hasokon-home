...
**C の完了確認は 2026-09-29 の週次監査**（末尾「経過」）。[google-index-recovery.md](./google-index-recovery.md) の「経過」
...
- 2026-09-24：**C を実施。** Search Console の画面で `/learn/sitemap.xml` と `/sitemap-home.xml` を
  ...
  - **C の完了判定**：**2026-09-29（月）の週次監査**で `/learn/sitemap.xml` と `/sitemap-home.xml` に
    `known: true` と `lastDownloaded` が入り、この 2 本による終了コード 1 が消えるか。
    消えなければ C は効いていない（本ファイルに追記して原因を調べる）
