...
**状態**：**A・B は実装済み（2026-09-19）。C（運営者の画面作業）も実施済み（2026-09-24）。**
**C の完了確認は 2026-09-28（月）の週次監査で最初に見て、10-05（月）で確定**（末尾「経過」）。[google-index-recovery.md](./google-index-recovery.md) の「経過」
...
- 2026-09-24：**C を実施。** Search Console の画面で `/learn/sitemap.xml` と `/sitemap-home.xml` を
  個別送信した（運営者の承認のもと AI エージェントが Chrome で操作。
  詳細は [google-index-recovery.md](./google-index-recovery.md) の「経過」2026-09-24）。
  送信直後の画面では `/learn/sitemap.xml` が「成功しました」検出 39、`/sitemap-home.xml` が
  「成功しました」検出 2（最終読み込みは 09/09 のまま）。
  - **C の完了判定**：週次監査（`gsc-audit.yml`、毎週月曜 09:00 JST）で `/learn/sitemap.xml` と
    `/sitemap-home.xml` に `known: true` と `lastDownloaded` が入り、この 2 本による終了コード 1 が消えるか。
    **2026-09-28（月）の監査で最初に確認**し（送信の 4 日後）、入っていなければ
    **10-05（月）の監査で確定**させる（上記「送信後 1 週間」の基準）。10-01 の 23 件再計測と並べて見る。
    10-05 でも消えなければ C は効いていない（本ファイルに追記して原因を調べる）
  - **A と C の切り分けは、結果的にできるようになった。** 当初は「同時に打つので区別しない」と
    決めていたが、実際は **A（`home/robots.txt`、#237）が v1.19.0 で 2026-09-19 に単独で本番に出て、
    C はその 5 日後**になった。その 5 日間の計測（#245）では learn の unknown は 39 → 38 で、
    **A 単独の効果はほぼゼロ**と観測済み。したがって **10-01 以降に learn の unknown が動いたら、
    それは C に帰属できる**
