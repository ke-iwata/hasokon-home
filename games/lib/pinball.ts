 ...
 * 台の壁。
 *
 * **オービット（周回レーン）とランプを持つ構成**にしてある。
 * 昔のパソコンに付属していた宇宙ものの台を思い出せる骨格——
 * 左の周回・上の通り道・右の上がりレーン——を、自前の寸法で組んだもの。
 * 名称・絵柄・意匠は真似ない（games/CLAUDE.md の商標の約束）。
 *
 * ```
 *   ┌──────天井────────┐
 *   │ ┌── 上の通り道 ──┐ \      ← 天井とガイドのあいだ。天井のレーン3つが並ぶ
 *   │ │              │  \
 *   │ │   バンパー     │   \
 *  左 │      ・的      │  打ち出しレーン
 *  周 │              ランプ │
 ...
 * ```
 *
 * **通り道の幅は球3つぶんを下回らせない。** 球の直径は 0.044、壁の太さが
 * 片側 0.012 あるので、見た目の幅から 0.024 引いた値が実際に通れる幅になる。
 * ここを詰めると球が引っかかって、突き（`restSec`）頼みの見苦しい動きになる。
 */
export const WALLS: Segment[] = [
  ...
  // 打ち出しレーン。**内側は y=0.34 から下だけ**。
  // ここに上まで壁を立てると、打ち出した球が盤面へ出られなくなる
  wall(0.97, 0.34, 0.97, 1.45),
  wall(LANE_LEFT, 0.34, LANE_LEFT, 1.45),

  // ---- 打ち出しレーンの口の一方通行の門（上りだけ通す）
  gate(LANE_LEFT, 0.3, 0.97, 0.3),

  // ---- 周回レーンの内側のガイド。
  // 天井・左の壁とのあいだが通り道になる。下（y=0.62）が口で、
  // **左へ打ち上げた球はここを上って天井を右へ抜ける**（＝周回）
  // 下端は外へ広げて口にする（直線で終わると球が角に当たって跳ね返るだけになる）
  wall(0.2, 0.71, 0.15, 0.6),
  wall(0.15, 0.6, 0.15, 0.3),
  wall(0.15, 0.3, 0.24, 0.17),
  wall(0.24, 0.17, 0.45, 0.15),
  // 右端は下へ折って、抜けた球を盤面（バンパー）へ落とす
  wall(0.45, 0.15, 0.64, 0.18),
  wall(0.64, 0.18, 0.71, 0.28),

  // ---- 右の戻りレーン。打ち出しレーンの内側に並ぶ2本目の通路で、
  // 上から降りてきた球を斜面（スリングショット）まで導く
  // **折り曲げないこと。** 途中で内側へ折ると、そこが球1つぶんの楔になって
  // 球が挟まる（実測：無操作で55秒動かなくなった。突きも壁側へ押すので出られない）
  wall(0.79, 0.34, 0.75, 0.98),
...
/**
 * 一方通行の門。**上っていく球は通し、降りてくる球は跳ね返す。**
 *
 * 打ち出しレーンの口に置く。これが無いと、周回して降りてきた球が
 * レーンへ落ちて「打ち直し」になり、**遊びが始まらないまま台を1周して終わる**
 * （実測：どの強さで打っても4秒でレーンに戻っていた）。
 * 本物の台にも同じ役割の逆流防止の羽根が付いている
 */
function gate(x1: number, y1: number, x2: number, y2: number): Segment {
  return { x1, y1, x2, y2, bounce: 0.3, kick: 0, kind: 'gate' };
}
...
export function buildBumpers(): Bumper[] {
  return [
    // 盤面の上、三角に3つ。周回から抜けた球も、フリッパーから上げた球もここへ入る
    { x: 0.52, y: 0.29, r: 0.055 },
    { x: 0.41, y: 0.44, r: 0.055 },
    { x: 0.63, y: 0.44, r: 0.055 },
    // 的とフリッパーのあいだの返し。ここが無いと落ちてくる球にさわれる場所が無い
    { x: 0.18, y: 0.92, r: 0.038 },
    { x: 0.72, y: 0.92, r: 0.038 },
  ];
}

/**
 * 的。**横一列ではなく、左に階段状**に置いてある。
 * 一列だと「どこに当てても同じ」になり、狙う面白さが出ない。
 * 段にすると、フリッパーから上げる角度で当たる的が変わる
 */
export function buildTargets(): Target[] {
  return [
    { x: 0.19, y: 0.78, w: 0.1, h: 0.028, down: false },
    { x: 0.26, y: 0.71, w: 0.1, h: 0.028, down: false },
    { x: 0.33, y: 0.64, w: 0.1, h: 0.028, down: false },
  ];
}
...
export function buildSaucer(): Saucer {
  return { x: 0.6, y: 0.62, r: 0.042, hold: 0, cool: 0 };
}

/** 天井のレーン。打ち出した球が回り込みから流れ込む道の途中に置く */
/**
 * 天井のレーン。**周回の通り道の中**に並べてある（天井とガイドのあいだ）。
 * 左へ打ち上げて周回させると通る＝「狙って回すと倍率が上がる」になる
 */
export function buildLanes(): Lane[] {
  return [0.26, 0.42, 0.58].map((x) => ({ x, y: 0.1, w: 0.13, lit: false }));
}
...
export function step(state: PinballState, dt: number, input: PinballInput): PinballState {
  ...
  for (let i = 0; i < SUBSTEPS; i += 1) {
    ...
    for (const w of WALLS) {
      // 一方通行の門。上っていく球（vy<0）には触らない
      if (w.kind === 'gate' && ball.vy < 0) continue;
  ...
  if (restSec > 1.5) {
    ...
    // 上向きを強めにしてある。横だけだと、壁ぎわで止まったとき壁へ押しつけて終わる
    ball.vy -= 0.45;
    restSec = 0;
  }
