...
export const FRUITS: FruitDef[] = [
  ...
  // **「なし」と色が重なっていたので、実の色（ルビー種の桃色）に振り直した。**
  // どちらも淡い黄で、暗い側は `#a16207` がまったく同じ・明るい側の差も10しか
  // なかった（大きさの比は1.4倍しかないので、実質見分けが付かない）。
  // 隣り合う段だけを見ていて、離れた段の組が抜けていた
  { name: 'グレープフルーツ', r: 0.136, light: '#fda4af', dark: '#881337', stem: '#4d7c0f', deco: 'stem' },
 ...
 *
 * **同じフレームで別々の組が2つ合体すると `chain` は2つ進む。**
 * 「合体が次の合体を呼ぶ」という連鎖の説明とは少しずれるが、これは承知のうえ。
 * 得点が甘くなる方向で遊ぶ人が損をせず、離れた場所で同時に起きた合体を
 * 「連鎖ではない」と分けるには合体どうしの因果を追う必要があって、
 * 見合う複雑さではないと判断した
 */
