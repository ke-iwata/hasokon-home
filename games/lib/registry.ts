...
export const games: GameDef[] = [
  ...
  {
    // スライドブロックパズルのルールは1900年代はじめの古典で、特許も意匠もとうに切れて
    // いる（docs/features/game-hakoiri-musume.md の「権利関係は無害」）。駒の呼び名と
    // 4×5の標準配置も伝統的なもので、特定社のものではない。市販品のパッケージ絵・
    // 書体・特定アプリの画面はなぞらず、色と文字だけの平面にしている。
    // **英語名「Klotski」は併記しない**（外国での登録状況を追えないため）。
    // **「箱入り娘」の名称の商標確認（J-PlatPat・第28類/第9類）は運営者のタスク**で、
    // 仕様書の状態行のとおり**着手前に確認して名称と slug を確定させる約束**。
    // 登録が見つかったら name を「娘を外に出すスライドパズル」、slug を
    // `slide-out-puzzle` に切り替える（`wip` のうちに済ませる。公開後はURLを動かせない）。
    // 同じ「盤を見て考える」型の色水ソート・ブロックパズルの隣に置く
    slug: 'hakoiri-musume',
    icon: 'SlidingBlocks',
    name: '箱入り娘',
    description:
      '4×5の盤で駒を滑らせ、大きな「娘」を下の出口まで運ぶ古典のスライドパズル。やさしい配置から伝統的な標準配置、むずかしい配置まで全10面。最短手数つき。',
    stage: 'wip',
  ...
  {
    // 落として同じもの同士を合体させるメカニクスはアイデアであって
    // 著作権の保護対象ではなく、同系のゲームが各ストアに多数ある。
    // ただし**「スイカゲーム」は Aladdin X 社の商品名（商標）**なので、
    // 名前・slug・見出し・説明のどこにも使わない。
    // **題材はボルダリングのホールド**で、種類名（ガバ・カチ・ピンチなど）は
    // クライミングの一般的な用語。**特定メーカーの商品ホールドの形はなぞらない**
    // （ホールドは造形物なので意匠の話になる。
    // docs/features/game-bouldering-merge.md の冒頭）。
    // **「ボルダリング」の商標確認は運営者のタスク**（J-PlatPat をこの環境から引けない）。
    // **公開の条件は仕様書の判定リスト**（1画面・60個で震えなし・1プレイ3〜10分・
    // 60fps）。満たしたら `preview` → `public` に上げる
    slug: 'bouldering-merge',
    icon: 'BoulderingMerge',
    name: 'ボルダリング合体パズル',
    description:
      '箱にクライミングのホールドを落として、同じホールドどうしをぶつけて大きく育てる物理パズル。カチからトップホールドまで11段階。当たり判定はホールドの形そのままです。',
    stage: 'wip',
