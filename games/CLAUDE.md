...
9. **演出はゲームの状態に持たせない。** 光り・残像・浮かぶ得点のような
   「当たった余韻」は `Game.tsx` の ref に置き、`lib/` の状態には入れない
   （ピンボールの `Fx`）。物理や進行の状態に演出を混ぜると、
   **テストがUIの都合で壊れるようになる**。どこが光るかは、状態にある位置
   （球の座標など）からその場で当てれば足りる
10. **局面を足したら不変条件のテストを直す。** 「48枚そろっている」のような確認は、
