...
const faq = [
  ...
  {
    q: '得点の仕組みは？',
    a: '丸いバンパーは100点、横に並ぶ4つの的は250点です。盤面のまんなかにある紫色の吸い込みホールに入ると500点で、球をしばらく抱えたあと上へ撃ち出されます。天井の3つのレーンは通るたびに50点。打ち出しの通路にあるスピナー（回転板）は、回っているあいだ半回転ごとに60点が入ります。フリッパーの上のピンク色の斜面（スリングショット）も、勢いよく当たると10点です。',
  },
  {
    q: '倍率はどうすれば上がりますか？',
    a: '上げ方は2つあります。横に並ぶ4つの的をすべて倒すか、天井の3つのレーンをすべて通すかで、それぞれボーナス1000点と倍率+1が入ります（最大5倍）。どちらも倍率が上がると元に戻るので、繰り返し狙えます。ただし球を落とすと倍率は1に戻ります。',
  },
  {
    q: 'スピナーはどうすれば回りますか？',
  ...
  return (
    <>
      ...
      <ol>
        ...
        <li>丸いバンパーは100点、横に並ぶ4つの的は250点、まんなかの吸い込みホールは500点</li>
        <li>的を4つ全部倒すか、天井の3つのレーンを全部通すと、ボーナス1000点で倍率が1つ上がります（最大5倍）</li>
      ...
      <ul>
        ...
        <li>
          倍率を上げる道は2つあります。<strong>的4つ</strong>と<strong>天井のレーン3つ</strong>で、
          レーンは打ち出した球が流れ込む場所なので、狙わなくてもそろうことがあります
        </li>
      </ul>
      ...
      <p>
        このページのピンボールは、プランジャー・フリッパー・バンパー・的・スピナー・
        吸い込みホール・天井のレーンで組んだオリジナルの台です。
        特定の製品の盤面や意匠は使っていません。
